...
function loadFolder(path) {
    ...
    if (path == '/') {
        $.ajax({
            ...
            data: "fields=name&filter=" + escape("type IN ('local_file','aws_s3','azure_blob','rackspace_cloud_files','openstack_object_storage','ftp_file','sftp_file','webdav_file')"),
            cache: false,
...
.config(['fileManagerConfigProvider', '$httpProvider', function (config, $httpProvider) {
  ...
  config.set({
    listUrl: "/api/v2/system/service?fields=name&filter=type%20IN%20('local_file'%2C'aws_s3'%2C'azure_blob'%2C'rackspace_cloud_files'%2C'openstack_object_storage'%2C'ftp_file'%2C'sftp_file'%2c'webdav_file')&_=1455310375411",
